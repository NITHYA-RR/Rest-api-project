...
class OAuth
{
    ...
    public function authenticate()
    {
        if ($this->access_token != null) {
            $query = "SELECT * FROM apis.session WHERE access_token = '" . mysqli_real_escape_string($this->db, $this->access_token) . "';";
            $result = mysqli_query($this->db, $query);
            if ($result) {
                $data = mysqli_fetch_assoc($result);
                if ($data && isset($data['created_at']) && isset($data['valid_for']) && isset($data['username'])) {
    ...
    {
        if ($this->refresh_token != null && !$this->startsWith($this->refresh_token, 'd.')) {
            $query = "SELECT * FROM apis.session WHERE refresh_token = '" . mysqli_real_escape_string($this->db, $this->refresh_token) . "';";
            $result = mysqli_query($this->db, $query);
            if ($result) {
                $data = mysqli_fetch_assoc($result);
                if ($data && isset($data['username']) && isset($data['valid']) && $data['valid'] == 1) {
